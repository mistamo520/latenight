...
class ContactToolController extends Controller
{
    public function load()
    {
        ...
        if (true || $_SERVER['SERVER_NAME'] == 'info.91supai.com') {
            ...
            $is_pc = $data['pc'] == 'pc';
            $is_mobile = !$is_pc;
            $user = User::get(['contact_tool_setting' => ['like', '%' . $data['key'] . '%'], 'type' => Constant::USER_TYPE_CLIENT]);
            if (empty($user) || strpos($data['domain'], $user->website) === false) {
                return null;
            } else {
                $setting = json_decode($user['contact_tool_setting'], true);
                $contact_enable = true;
                $contact_float_enable = true;
                $message_enable = true;
                $message_call_enable = false;
                ...
                $contact_code = $setting['contact_code'];
                $copy_code = $setting['copy_code'];
                $agent = User::get($user->parent_id);
                $system_name = empty($agent->oem_name) ? '全网竞价' : $agent->oem_name;
                $expired = false;
                if ($expired) {
                    $contact_code = '';
                    $copy_code = '';
                    ...
                    if (!empty($setting['message_call_enable'])) {
                        $message_call_enable = true;
                    }
                ...
                return api_success('', ['expired' => $expired, 'contact_code' => $contact_code, 'copy_code' => $copy_code, 'theme_color' => $theme_color, 'message_title' => $message_title, 'message_enable' => $message_enable, 'message_call_enable' => $message_call_enable, 'message_position' => $message_position, 'contact_enable' => $contact_enable, 'contact_open_message' => $contact_open_message, 'contact_float_position' => $contact_float_position, 'contact_float_enable' => $contact_float_enable, 'message_open_seconds' => $message_open_seconds, 'contact_open_seconds' => $contact_open_seconds, 'system_name' => $system_name]);
            }
        }
    }
    public function message()
    {
        header('Access-Control-Allow-Origin:*');
        $data = input('param.');
        $is_call = !empty($data['is_call']);
        if (empty($data['key']) || empty($data['domain'])) {
            return null;
        } elseif ($is_call && empty($data['phone'])) {
            return null;
        } elseif (!$is_call && (empty($data['phone']) || empty($data['name']) || empty($data['content']))) {
            return null;
        } else {
            if ($is_call) {
                $data['name'] = '';
            ...
            if (empty($user) || strpos($data['domain'], $user->website) === false || empty($user->expired_date) || strtotime($user->expired_date) < time()) {
                ...
                if ($user->sms_count > 0) {
                    ...
                    if ($is_call) {
                        $content = '提醒您 - 紧急求购' . $data['phone'] . '，请及时回电，以免错过商机！';
                    } else {
